...
- **说人话，术语为理解服务**：先用自然的中文把意思说清；中文语境里常见、表达更准确的专业术语可以用，并按需以括号标注补充；术语不替代解释，也不引入中文语境里生僻的直译词
...
- **拿不准、纯偏好、已过时的问题直接省略**，不要用一长串 caveat 把不确定写成「也算规范」。
- **事实与偏好分开写**：规范依据、工具链实测是事实；标点、72 字符、emoji、`--no-ff` 这类是偏好。偏好要标明，不要扮成铁律。
