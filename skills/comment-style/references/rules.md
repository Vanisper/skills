...
- **源码注释**：力求精炼，只写契约、约束、边界、原因，不复述代码；中文注释一般不以句号结尾——句号是「成篇文章」的标点，而 API 注释是结构化卡片（见「域模型与空行」）：title 是定位短语、分点是精练碎片、signature 是契约事实，都不是句子；完整句子只通行于实现内的流程注释
...
API 块注释是结构化卡片，不是文章。按内容职责分域，域之间空一行：
...
- **example / see 域**（按需）：`@example` 使用示例、`@see` / `@link` 关联引用
